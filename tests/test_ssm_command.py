...
class SSMCommand(unittest.TestCase):
    def setUp(self):
        self.env = {'INSTANCE_ID': 'i-08d8c71afdc921985',
                    ...
                    'GITHUB_SHA': 'a' * 40}
        for service in ['frontend', 'backend', 'ai']:
            self.env[service.upper() + '_IMAGE'] = ('764788758503.dkr.ecr.ap-northeast-2.amazonaws.com/mme-'
                                                   + service + ':' + 'b' * 40)
    ...
    def test_reject_untrusted_input(self):
        for key, value in [('INSTANCE_ID', 'i-foo; id'), ('GITHUB_SHA', 'main'),
                           ...
                           ('BACKEND_IMAGE', 'other.example/app:latest')]:
